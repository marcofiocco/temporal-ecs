...
[ -z "$2" ] && echo "No Permission Boundary argument supplied" && exit 1
[ -z "$3" ] && echo "No Resource Tag argument supplied" && exit 1
...
copilot app init --permissions-boundary $PermissionBoundary --resource-tags $ResourceTag
...
copilot svc init -a "$AppName" -t "Load Balanced Web Service" -n "$SvcName" # this creates a public applcation or network load balancer, ECS service and addons

copilot svc init -a "$AppName" -t "Load Balanced Web Service" -n "$UISvcName" # this creates a public applcation or network load balancer, ECS service and addons
